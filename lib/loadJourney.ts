...
import type { Journey, Step, Option, Product, StepProduct } from "@/lib/supabase";
...
export async function loadJourney(slug: string): Promise<LoadedJourney | null> {
  ...
  const { data: steps } = await supabase
    .from("steps")
    .select(
      "id, journey_id, type, title, question_text, video_url, position, buttons_layout, button_template, button_color, button_opacity, button_font_color, button_font, button_border_color, button_shadow, buttons_reveal_enabled, buttons_reveal_seconds, question_position, question_font_size, question_font_color, question_bg_enabled, question_bg_color, button_text_size"
    )
